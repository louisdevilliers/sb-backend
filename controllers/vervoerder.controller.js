...
const { vervoerders: Vervoerder, Sequelize: { Op } } = db;

// Create and Save a new Vervoerder
export const create = async (req, res) => {
  ...
  Vervoerder.create(vervoerder)
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the Vervoerder."
      });
    });
};
...
export const findAll = async (req, res) => {
    ...
    Vervoerder.findAll({ where: condition })
      .then(data => {
        res.send(data);
      })
      .catch(err => {
